class Parallax {
	constructor(elements, options = {}) {
		this.parallaxLayers = Array.from(elements).map(
			(element) => new Parallax.Layer(element, options)
		);
	}
...
Parallax.Layer = class {
	...
	animationFrame() {
		...
		const positionY =
			(this.mouseY * this.height) / 10 + Math.max(scrollChange / 3, 0);
		// для плавности
